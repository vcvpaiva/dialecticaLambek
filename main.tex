...
$(\mathcal{M}, \leq, \otimes, I)$ is a ordered non-commutative monoid.
Now we can define two new operations on $\mathcal{M}$, $(-) \lto (-)$
and $(-) \rto (-)$, both of the form $\mathcal{M}^{op} \times
\mathcal{M} \mto \mathcal{M}$ by $\alpha \lto \beta = \alpha^{op}
\otimes \beta$ and $\alpha \rto \beta = \beta^{-1} \otimes \alpha$.
...
if one has an $n$-ordered group, i.e. the possibility of inverting
elements of an non-symmetric ordered monoid, we will have a biclosed
poset.
