...
sequel.  We show that the set of invertible matrices over the set of
natural numbers, $\mathbb{N}$, is a biclosed poset, and that
$\mathbb{N}$ is itself a biclosed poset.

\subsubsection{The Set of Invertible Matrices as a Model for L}
\label{subsec:the_set_of_invertible_matrices}

Consider the set $\mathcal{M}$ of square invertible matrices with
...
\begin{itemize}
\item it is non-commutative which means that $\alpha \otimes \beta
...
\alpha^{op} \otimes \beta$ and $\alpha \rto \beta = \beta^{-1} \otimes
\alpha$.  Thus, we now have the following:
\begin{lemma}
  \label{lemma:M-is-biclosed-poset}
  The ordered monoid of invertible $n \times n$ matrices over
...
\subsubsection{The Set of Natural Numbers as a Model for LP}
\label{subsec:the_set_of_natural_numbers}
The calculus with permutations, $\textbf{LP}$, has a model in the
natural numbers; as well as the reals.  The set of natural numbers is
clearly a symmetric closed poset, where conjunction is $0$ and the
internal-hom is given by truncated subtraction, as in Lawvere
\cite{lawvere1973} (also in de Paiva \cite{Hyland:1991}).  It is easy
to prove the adjunction and another easy argument on Lindenbaum
algebras (following Girard's proof) proves completeness of the models
above.
